...
$result = $conn->query("SELECT * FROM users WHERE role IN ('lab', 'pharmacy')");
...
<h4>Pending/Approved Labs, Pharmacies & Users</h4>
...
<table id="usersTable" class="table table-bordered table-hover table-striped align-middle">
    ...
    <?php if (!empty($row['logo_path'])): ?>
        <img src="../<?= htmlspecialchars($row['logo_path']) ?>" alt="Logo" style="max-width: 80px; max-height: 80px;">
    <?php else: ?>
        No Logo
    <?php endif; ?>
</td>
...
<td><?= htmlspecialchars($row['address']) ?></td>
            ...
            <td><?= $row['license_expiry_date'] ? date('d-M-Y', strtotime($row['license_expiry_date'])) : '-' ?></td>
